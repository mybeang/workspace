...
class Form(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        ui_file = QtCore.QFile("windows.ui")
        """
        self.ui = uic.loadUi('./Window.ui',
                             self)  # If the script is not working, please check the file directory, and Python env path
    ...
    @Slot()  # This is to establish a connection via Telnet at this version. In the furture, Netconf/SSH module
    # would be added.
    def slot_2st(self):
    ...
    def Tab1_running(self, args):
        ...
        if ((date_from or date_to) == '2000-01-01 00:00.00') or (date_from <= QDateTime.currentDateTime().toString(
                "yyyy-MM-dd hh:mm:ss") and QDateTime.currentDateTime().toString("yyyy-MM-dd hh:mm:ss") <= date_to):
            for i in range(int(self.lineEdit_5.text())):
                ...
                if ((date_from or date_to) == '2000-01-01 00:00.00') or (
                        ...
                        "yyyy-MM-dd hh:mm:ss") and QDateTime.currentDateTime().toString(
    ...
    @Slot()  # This slot is for  "Running or Enter key" button.
    def slot_3st(self):
        self.t = threading.Thread(target=self.Tab1_running, args=(
        "task",))  # for multithreading, currently 'args' is for feature usage. Not used yet.
        self.t.start()

    @Slot()  # This slot is for  "Stop or Ctrl+c" button.
    def slot_4st(self):
